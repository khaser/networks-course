...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - 36 байт на полезную нагрузку (8 байт на UDP заголовки и 28 байт на UDP данные)
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - В моём случае отправляются UDP дейтаграммы. У каждой дейтаграммы уникальный заголовок Identification (не считая header checksum).
        Помимо этого, каждые 3 пакета увеличивается TTL, что и ожидается.
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Остальные поля остаются неизменными, это Protocol, Source address, Destination address, Flags и прочие.
      - В целом такое поведение и ожидается: всегда меняется Identification и TTL увеличивается раз в три пакета т.к. `traceroute` в UNIX системах отправляет по 3 сэмпла с одинаковым TTL, чтобы точнее понять среднее время до целевого хоста.
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Закономерность в Identification обнаружить не удалось.
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Среди пакетов, поступившие от ближайшего маршрутизатора все пакеты имеют TTL=64 и различные Identification = 785, 786 и 787 соответственно.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами?
      ...
      - Да, сообщение было фрагментировано на 3 дейтаграммы.
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Изменяется поле Fragment offset, которое позволяет ассемблировать разделённые пакеты на хосте-адресате. Также изменяется поле Header checksum.
