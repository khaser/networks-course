...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - Transaction ID: 0x9fb706cd (одинаковый и для запроса и для ответа). Данное поле используется для сопоставления DHCP ответа ранее
     отправленному DHCP запросу.
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   ...
   - IP Address Lease Time: (7200s) 2 hours
       Ограничение срока аренды IP адреса используется для того, чтобы пространство локальных IP адресов, которое обычно небольшое,
       не засорялось клиентами, которые уже давно не используют нашу локальную сеть.
...
#### Демонстрация работы
<img src="images/crc.png" width=700 />
