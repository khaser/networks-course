...
#### Демонстрация работы
Ниже приведён пример запуска в двух консолях, который передаёт файл
`example.txt` от клиента серверу(`example_received_by_client.txt`) и обратно(`example_received_by_server.txt`).
На снимках видно, что эти файлы не отличаются не смотря на то, что в эмитируемой среде передачи 30%
пакетов теряются при передаче.
<img src="images/client.png" width=700 />
<img src="images/server.png" width=700 />
